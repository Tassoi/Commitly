...
export function TopBar() {
  ...
  const handleSelectFromHistory = async (path: string, name: string) => {
    try {
      const loadingToast = toast.loading(t('正在打开仓库名', { name }));
      const info = await openRepository(path);
      const repoId = addRepoToHistory(info);
  ...
  return (
    <header className="h-14 border-b bg-background flex items-center justify-between px-6">
      <div className="flex items-center gap-2">
        {activeRepos.size === 0 ? (
          <span className="text-sm text-muted-foreground">{t('未选择仓库')}</span>
        ) : activeRepos.size === 1 ? (
          <>
            <span className="text-sm text-muted-foreground">{t('当前仓库')}</span>
            <span className="font-medium">{Array.from(activeRepos.values())[0].repoInfo.name}</span>
          </>
        ) : (
          <span className="font-medium">{t('个活动仓库', { count: activeRepos.size })}</span>
        )}
      </div>
      <div className="flex items-center gap-2">
        <Popover>
